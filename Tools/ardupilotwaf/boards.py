...
class Board:
    ...
    def configure(self, cfg):
        ...
        if (cfg.options.enable_custom_controller or self.get_name() == "sitl") and not cfg.options.no_gcs:
            ...
            cfg.msg("Enabled PCAC", "yes")
        else:
            env.DEFINES.update(
                AP_PCAC_ENABLED=0,
            )
            cfg.msg("Enabled PCAC", "no", color="YELLOW")

        if cfg.options.soh_ranger_1600:
            env.DEFINES.update(
                SOH_RANGER_1600=1,
            )
            cfg.msg("Ranger 1600 Sim-On-Hardware", "yes")
        else:
            env.DEFINES.update(
                SOH_RANGER_1600=0,
            )
            cfg.msg("Ranger 1600 Sim-On-Hardware", "no", color="YELLOW")
        if cfg.options.soh_ranger_ex:
            env.DEFINES.update(
                SOH_RANGER_EX=1,
            )
            cfg.msg("Ranger EX Sim-On-Hardware", "yes")
        else:
            env.DEFINES.update(
                SOH_RANGER_EX=0,
            )
            cfg.msg("Ranger EX Sim-On-Hardware", "no", color="YELLOW")
...
class SITLBoard(Board):
    ...
    def configure_env(self, cfg, env):
        ...
        if self.with_can:
            ...
            env.DEFINES.update(CANARD_MULTI_IFACE=1, CANARD_IFACE_ALL=0x3, CANARD_ENABLE_CANFD=1, CANARD_ENABLE_ASSERTS=1)

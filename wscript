...
def add_build_options(g):
    ...
    for opt in build_options.BUILD_OPTIONS:
        ...
        g.add_option(enable_option, action="store_true", default=False, help=enable_description)
        g.add_option(disable_option, action="store_true", default=False, help=disable_description)
        ...
        if lower_enable_option != enable_option:
            g.add_option(lower_enable_option, action="store_true", default=False, help=optparse.SUPPRESS_HELP)
        lower_disable_option = disable_option.lower().replace("_", "-")
        if lower_disable_option != disable_option:
            g.add_option(lower_disable_option, action="store_true", default=False, help=optparse.SUPPRESS_HELP)
...
def add_script_options(g):
    ...
    for d in driver_list + applet_list:
        ...
        g.add_option(embed_option, action="store_true", default=False, help="Embed %s in ROMFS" % bname)
...
def options(opt):
    ...
    g.add_option(
        "--board", action="store", default=None, help="Target board to build, choices are %s." % ", ".join(boards_names)
    )

    g.add_option("--debug", action="store_true", default=False, help="Configure as debug variant.")

    g.add_option("--debug-symbols", "-g", action="store_true", default=False, help="Add debug symbolds to build.")
    ...
    g.add_option("--disable-watchdog", action="store_true", default=False, help="Build with watchdog disabled.")

    g.add_option("--coverage", action="store_true", default=False, help="Configure coverage flags.")

    g.add_option("--Werror", action="store_true", default=None, help="build with -Werror.")

    g.add_option("--disable-Werror", action="store_true", default=None, help="Disable -Werror.")
    ...
    g.add_option("--enable-asserts", action="store_true", default=False, help="enable OS level asserts.")

    g.add_option("--save-temps", action="store_true", default=False, help="save compiler temporary files.")

    g.add_option("--enable-malloc-guard", action="store_true", default=False, help="enable malloc guard regions.")

    g.add_option("--enable-stats", action="store_true", default=False, help="enable OS level thread statistics.")

    g.add_option("--bootloader", action="store_true", default=False, help="Configure for building a bootloader.")

    g.add_option("--signed-fw", action="store_true", default=False, help="Configure for signed firmware support.")

    g.add_option("--private-key", action="store", default=None, help="path to private key for signing firmware.")
...
""",
    ...
    g.add_option("--enable-header-checks", action="store_true", default=False, help="Enable checking of headers")

    g.add_option("--default-parameters", default=None, help="set default parameters to embed in the firmware")

    g.add_option("--enable-math-check-indexes", action="store_true", default=False, help="Enable checking of math indexes")

    g.add_option("--disable-scripting", action="store_true", default=False, help="Disable onboard scripting engine")

    g.add_option("--enable-scripting", action="store_true", default=False, help="Enable onboard scripting engine")

    g.add_option("--no-gcs", action="store_true", default=False, help="Disable GCS code")

    g.add_option("--scripting-checks", action="store_true", default=True, help="Enable runtime scripting sanity checks")

    g.add_option("--enable-onvif", action="store_true", default=False, help="Enables and sets up ONVIF camera control")

    g.add_option("--scripting-docs", action="store_true", default=False, help="enable generation of scripting documentation")

    g.add_option("--enable-opendroneid", action="store_true", default=False, help="Enables OpenDroneID")

    g.add_option("--enable-check-firmware", action="store_true", default=False, help="Enables firmware ID checking on boot")

    g.add_option("--enable-custom-controller", action="store_true", default=False, help="Enables custom controller")

    g.add_option(
        "--soh-ranger-1600",
        action="store_true",
        default=False,
        help="Modifies SITL servo direction for Sim-On-Hardware ranger 1600 frame",
    )
    ...
    g.add_option(
        ...
        action="store_true",
        default=False,
        help="Modifies SITL servo direction for Sim-On-Hardware ranger ex frame",
    )

    g.add_option("--enable-gps-logging", action="store_true", default=False, help="Enables GPS logging")

    g.add_option("--disable-networking", action="store_true", help="Disable the networking API code")
    ...
    g.add_option("--enable-dronecan-tests", action="store_true", default=False, help="Enables DroneCAN tests in sitl")
...
""",
    ...
    g.add_option("--enable-benchmarks", action="store_true", default=False, help="Enable benchmarks.")

    g.add_option("--enable-lttng", action="store_true", default=False, help="Enable lttng integration")
    ...
    g.add_option("--disable-tests", action="store_true", default=False, help="Disable compilation and test execution")

    g.add_option("--enable-sfml", action="store_true", default=False, help="Enable SFML graphics library")

    g.add_option("--enable-sfml-joystick", action="store_true", default=False, help="Enable SFML joystick input library")

    g.add_option("--enable-sfml-audio", action="store_true", default=False, help="Enable SFML audio library")

    g.add_option("--osd", action="store_true", default=False, help="Enable OSD support")

    g.add_option("--osd-fonts", action="store_true", default=False, help="Enable OSD support with fonts")

    g.add_option("--sitl-osd", action="store_true", default=False, help="Enable SITL OSD")

    g.add_option("--sitl-rgbled", action="store_true", default=False, help="Enable SITL RGBLed")

    g.add_option("--force-32bit", action="store_true", default=False, help="Force 32bit build")
    ...
    g.add_option("--sitl-flash-storage", action="store_true", default=False, help="Use flash storage emulation.")

    g.add_option("--ekf-double", action="store_true", default=False, help="Configure EKF as double precision.")

    g.add_option("--ekf-single", action="store_true", default=False, help="Configure EKF as single precision.")

    g.add_option("--static", action="store_true", default=False, help="Force a static build")

    g.add_option("--postype-single", action="store_true", default=False, help="force single precision postype_t")
    ...
    g.add_option("--extra-hwdef", action="store", default=None, help="Extra hwdef.dat file for custom build.")

    g.add_option("--assert-cc-version", default=None, help="fail configure if not using the specified gcc version")

    g.add_option("--num-aux-imus", type="int", default=0, help="number of auxiliary IMUs")

    g.add_option("--board-start-time", type="int", default=0, help="zero time on boot in microseconds")

    g.add_option("--enable-iomcu-profiled-support", action="store_true", default=False, help="enable iomcu profiled support")
